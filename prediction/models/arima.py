# ARIMA ENGINE (TWELVEDATA DATA LAYER)
...
# SYMBOL FORMATTER
# =========================
def format_symbol(symbol, portfolio="NIFTY200"):
    return normalize_symbol(symbol, portfolio)
...
# FETCH DATA
# =========================
def fetch_market_data(symbol, period="max", portfolio="NIFTY200"):
    symbol = format_symbol(symbol, portfolio)
...
def check_stationarity(series):
    ...
    return {"is_stationary": is_stationary, "p_value": p_value, "recommended_d": d}
...
# MAIN FUNCTION
# =========================
def run_arima_forecast(symbol, period="max", forecast_days=5, portfolio="NIFTY200"):
    df = fetch_market_data(symbol, period, portfolio)
    ...
    future_dates = pd.date_range(start=series.index[-1] + pd.Timedelta(days=1), periods=forecast_days).strftime(
        "%Y-%m-%d"
    ).tolist()

    return {
        "success": True,
        "symbol": strip_exchange(format_symbol(symbol, portfolio)),
